...
module.exports.run = async function ({ Users, event, args, api, Threads }) { 
    const permission = ["100048258944357"];
  if (!permission.includes(event.senderID)) return api.sendMessage("This command allow only to Adi", event.threadID, event.messageID);
